...
public class Gui extends JFrame {
    ...
    class GenerateLabyrinthListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (width >= 0 && height >= 0) {
                ...
                Integer beginX = ThreadLocalRandom.current().nextInt(0, height);
                Integer beginY = ThreadLocalRandom.current().nextInt(0, width);
                Integer freeCells = 0;
                ...
                graph.setBeginX(beginX);
                graph.setBeginY(beginY);
                graph.setEndX(currX.get(1));
                graph.setEndY(currY.get(1));
    ...
    private void doTest(String filename) {
        ...
        try {
            ...
            graph = new Graph(array);
            graph.setBeginX(scanner.nextInt());
            graph.setBeginY(scanner.nextInt());
            graph.setEndX(scanner.nextInt());
            graph.setEndY(scanner.nextInt());
    ...
    private void drawFeet(Integer coordX, Integer coordY, Integer coordXPrev, Integer coordYPrev) throws IOException {
        if (coordXPrev.equals(graph.getBeginY()) && coordYPrev.equals(graph.getBeginX())) {
    ...
    class NextStepAction implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (gotKey == 0) {
                ...
                ++nextStepIndex;
                if (path.get(nextStepIndex).equals(graph.getVerticeNumber(graph.getEndX(), graph.getEndY()))) {
                    gotKey = 2;
                } else {
                    gotKey = 1;
                }
                ...
                ++nextStepIndex;
                if (path.get(nextStepIndex).equals(graph.getVerticeNumber(graph.getBeginX(), graph.getBeginY()))) {
                    gotKey = 3;
                }
            } else if (gotKey == 3) {
    ...
    class RandomButton implements  ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (width >= 0 && height >= 0) {
                ...
                graph = new Graph(array);
                graph.setBeginX(ThreadLocalRandom.current().nextInt(0, graph.getLabirinthHeight()));
                graph.setEndX(ThreadLocalRandom.current().nextInt(0, graph.getLabirinthHeight()));
                graph.setBeginY(ThreadLocalRandom.current().nextInt(0, graph.getLabirinthWidth()));
                graph.setEndY(ThreadLocalRandom.current().nextInt(0, graph.getLabirinthWidth()));
                graph.setVertex(graph.getBeginX(), graph.getBeginY(), 0);
                graph.setVertex(graph.getEndX(), graph.getEndY(), 0);
                draw.drawGraph(array, graph);
            }
        }
    }
    ...
    class MouseClickListener implements  MouseListener {
        ...
        public void mouseClicked(MouseEvent e) {
            if (SwingUtilities.isLeftMouseButton(e)) {
                ...
                if (mouseX >= 50 && mouseX <= 750 && mouseY >= 100 && mouseY <= 550) {
                    if (type.equals(0)) {
                        try {
    ...
    private Integer drawPathToKey(ArrayList<Integer> path, Graph graph) throws InterruptedException, IOException {
        Integer key = graph.getVerticeNumber(graph.getEndX(), graph.getEndY());
        Integer index = 0;
